...
var Mockup = /** @class */ (function () {
    ...
    Mockup.apiAdd = function (req, res, next) {
        return __awaiter(this, void 0, void 0, function () {
            var data, doctors, clinics, users, visits;
            return __generator(this, function (_a) {
                try {
                    data = JSON.parse(fs_1.default.readFileSync('./src/mockup/doctors.json').toString());
                    doctors = data.map(function (d) {
                        d._id = new mongodb_1.ObjectId(d._id.$oid);
                        d.clinics = d.clinics.map(function (c) { return new mongodb_1.ObjectId(c.$oid); });
                        return d;
                    });
                    doctorsDAO_1.DoctorsDAO.addMany(doctors);
                    data = JSON.parse(fs_1.default.readFileSync('./src/mockup/clinics.json').toString());
                    clinics = data.map(function (c) {
                        c._id = new mongodb_1.ObjectId(c._id.$oid);
                        return c;
                    });
                    clinicsDAO_1.ClinicsDAO.addMany(clinics);
                    ...
                    users = data.map(function (u) {
                        if (u.hasOwnProperty('doctorId')) {
                            u.doctorId = new mongodb_1.ObjectId(u.doctorId.$oid);
                        }
                        u._id = new mongodb_1.ObjectId(u._id.$oid);
                        return u;
                    });
                    usersDAO_1.UsersDAO.addMany(users);
                    data = JSON.parse(fs_1.default.readFileSync('./src/mockup/visits.json').toString());
                    visits = data.map(function (v) {
                        v._id = new mongodb_1.ObjectId(v._id.$oid);
                        v.startDate = new Date(v.startDate.$date);
                        v.endDate = new Date(v.endDate.$date);
                        v.clinic = new mongodb_1.ObjectId(v.clinic.$oid);
                        v.doctor = new mongodb_1.ObjectId(v.doctor.$oid);
                        if (v.hasOwnProperty('appointment')) {
                            v.appointment._id = new mongodb_1.ObjectId(v.appointment._id.$oid);
                        }
                        return v;
                    });
